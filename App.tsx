...
const App: React.FC = () => {
  ...
  // Local atmospheric track
  const MUSIC_URL = "/asserts/springsnow.mp4";
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden">
      ...
      <BackgroundMusic isPlaying={isMusicPlaying} src={MUSIC_URL} />
      ...
      <div className={`absolute top-6 right-6 md:top-10 md:right-10 z-30 pointer-events-auto flex flex-row md:flex-col items-center md:items-end gap-3 md:gap-4 transition-opacity duration-500 ${isSignatureOpen || isProcessing ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
          ...
          <button 
            ...
            className={`${iconButtonClass} ${isMusicPlaying ? 'text-white border-white/60 bg-white/10' : 'text-slate-300'} ${!isMusicPlaying ? 'animate-pulse' : ''}`}
          ...
          >
              {isMusicPlaying ? (
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.2} stroke="currentColor" className="w-5 h-5 md:w-6 md:h-6">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M22 17.5L11 6.5M11 6.5V2.25L20.25 5.25v6.553M11 6.5l-2.25-2.25M20.25 5.25L3 21m16.25-3.377a2.25 2.25 0 01-1.632 2.163l-1.32.377a1.803 1.803 0 11-.99-3.467l2.31-.66a2.25 2.25 0 001.632-2.163V12.75l-4.5-1.543M9 15.553v3.75a2.25 2.25 0 01-1.632 2.163l-1.32.377a1.803 1.803 0 11-.99-3.467l2.31-.66A2.25 2.25 0 009 15.553z" />
                  </svg>
              )}
          </button>
